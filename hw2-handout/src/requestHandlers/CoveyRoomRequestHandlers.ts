...
export async function roomListHandler(): Promise<ResponseEnvelope<RoomListResponse>> {
  const roomsStore = CoveyRoomsStore.getInstance();
  const coveyRoomList = roomsStore.getRooms();
  return {
    isOK: true,
    response: {
      rooms: coveyRoomList,
...
export async function roomCreateHandler(requestData: RoomCreateRequest): Promise<ResponseEnvelope<RoomCreateResponse>> {
  const roomsStore = CoveyRoomsStore.getInstance();
  if (!requestData.friendlyName) {
    return {
      isOK: false,
      message: 'Error: the name for the new room is empty',
    };
  }
  const newController = roomsStore.createRoom(requestData.friendlyName, requestData.isPubliclyListed);
  return {
    isOK: true,
    response: {
      coveyRoomID: newController.coveyRoomID,
      coveyRoomPassword: newController.roomUpdatePassword,
    },
  };
}
...
export async function roomDeleteHandler(requestData: RoomDeleteRequest): Promise<ResponseEnvelope<Record<string, null>>> {
  const roomsStore = CoveyRoomsStore.getInstance();
  const deletedRoom = roomsStore.deleteRoom(requestData.coveyRoomID, requestData.coveyRoomPassword);
...
export async function roomUpdateHandler(requestData: RoomUpdateRequest): Promise<ResponseEnvelope<Record<string, null>>> {
  ...
  const updateRoom = roomsStore.updateRoom(requestData.coveyRoomID, requestData.coveyRoomPassword, requestData.friendlyName, requestData.isPubliclyListed);
  if (updateRoom === false) {
    return {
      isOK: false,
      message: 'Invalid Request',
