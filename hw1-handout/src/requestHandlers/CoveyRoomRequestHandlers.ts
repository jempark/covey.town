...
export async function roomJoinHandler(requestData: RoomJoinRequest): Promise<RoomJoinResponse> {
  const coveyRoomsStore = CoveyRoomsStore.getInstance();
  const coveyRoomController = coveyRoomsStore.getControllerForRoom(requestData.coveyRoomID);
...
export function roomSubscriptionHandler(socket: Socket): void {
  ...
  const coveyRoomsStore = CoveyRoomsStore.getInstance();
  const controller1 = coveyRoomsStore.getControllerForRoom(coveyRoomID);
